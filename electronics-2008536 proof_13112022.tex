...
\address{%
$^{1}$ \quad Swedish Transport Administration, Neptunigatan 52, P.O. Box 366, SE-201-23 Malm\"{o}, Sweden; %MDPI: English writing is recommended, please try to provide. -- Answer: it is the official name of the city, we cannot change it: https://en.wikipedia.org/wiki/Malm%C3%B6
 per.lindh@trafikverket.se or  per.lindh@byggtek.lth.se\\
$^{2}$ \quad Division of Building Materials, %MDPI: Address information should be sorted from subordinate to superior, please confirm this revision, the same as aff 3 -- Answer: yes, that is correct, we checked: Division of Building Materials is a part of the Department of Building and Environmental Technology of the Faculty of Engineering of Lund University. Also correct for the aff 3.
...
$^{3}$ \quad Laboratory of Image Synthesis and Analysis (LISA), \'{E}cole polytechnique de Bruxelles (Brussels Faculty of Engineering),  Universit\'{e} Libre de Bruxelles (ULB),  Building L, Campus de Solbosch, ULB---LISA CP165/57, Avenue~Franklin D. Roosevelt 50, B-1050 Brussels, Belgium}
...
\begin{itemize}
     \item Soil type A had the following characteristics: particle density $\rho _s =2.70~\text{mg/m}^3$; the optimum moisture content (OMC) according to the modified Proctor compaction test was 6.1\%; the dry density was $\rho _d = 2.26 ~\text{mg/m}^3$; %MDPI: Please confirm if ths unit Mg/m should be mg/m -- Answer: yes, corrected as you suggested.
...
%    \newcolumntype{C}{>{\centering\arraybackslash}X}
    ...
    \footnotesize{Notations for Table~\ref{tab:1}. %MDPI: Please confirm if the italics should be retained. -- Answer: ok, italics is removed (it was not necessary).
...
\bottomrule
    ...
    \footnotesize{Notations for Table~\ref{tab:2}.  %MDPI: Please confirm if the italics should be retained. -- Answer: ok, italics is removed (it was not necessary).
     Definitions of symbols, according to USCS: G: gravel (crushed rock); GW: well-graded gravel; GM: silty/clayey gravel; S: sand; CS: sandy/silty clays; CH: fat heavy clay. %MDPI: There is no ** in this table, please confirm if this can be removed -- Answer: ok, removed (it was not necessary).
...
\caption{Reaction on stabilisation from main soil compounds and suggested~binders}
 ...
 \toprule
        ...
        Sand & $SiO_2$ & Clay loam; asphalt \\
        Allophane & $Al_2O_3·(SiO_2)_{1.3-2}·(2.5-3)H_2 O$ %MDPI: Please confirm if the format is correct. -- Answer: corrected.
        & Lime (CaO) \\ 
        Kaolinite & $Al_2(OH)_4 Si_2 O_5$ & Sand; cement; lime \\ 
        Illite & $K,H_3 O(Al,Mg,Fe)_2 (Si,Al)_4 O_{10} [(OH)_2,(H_2O)$ & Cement; lime \\
        Montmorillonite & $(Na,Ca)_{0.33}(Al,Mg)_2 (Si_4 O_{10})(OH)_2·nH_2 O$ & Lime (CaO) \\ 
        \bottomrule
...
The {soil specimens} were treated by surface compaction using the {devices} available at SGI to compact the granular material of soil. A quantity of ~4.5 kg %MDPI: We changed the commas appearing between the digits into decimal dots. Please confirm this revision. -- Yes, corrected as you did.
...
\begin{figure}[H]
     ...
     \caption{{Comparison %MDPI: Please use periods as decimal sign instead of center dot. -- Answer: Fig. 2 is corrected as you requested (decimal sign instead of center dot in Y-axis).
...
\vfill \vspace{1mm}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \label{fig:03c}
     \end{subfigure}
        \caption{Variations in natural water content {by} soil~types.}%MDPI:  %MDPI: Please use periods as decimal sign instead of center dot. -- Answer: Fig. 3 is corrected as you requested (decimal sign instead of center dot in X-axis).
        \label{fig:03}
\end{figure}
...
%      	\centering
     ...
     \caption{{Equilateral triangle showing a }simplex lattice {mixture} design for binder selection (3, 2)} \label{fig:04}
\end{figure}%MDPI: Please use periods as decimal sign instead of center dot. -- Answer: Fig. 4 is corrected (now decimal sign instead of center dot).
...
\subsection{MCV}
{Equations} (\ref{eq:06}) and (\ref{eq:07}) {refer to the} response surface equation for binder blends tested for {various} soil types. Here, \emph{C, L} and \emph{S} %MDPI: Please check through the paper if the format of variables should be unified (normal or italic). -- Answer: ok, variables are checked throughout the paper and corrected to italics (soil types A, B and C and all combinations of variables are now in italics)
stand for cement, lime and steel slag, and~the combinations \emph{LS} and \emph{CLS} for their mixes, respectively. The~numbers represent the quantities of binders taken for stabilisation.
...
Nevertheless, {binder blends contributed significantly} to the MCV{. Thus, }the {combination} of blend OPS/GGBFS 50/50\% {increased} the MCV{ }up to 0.925 at a maximum for type \emph{A} (Equation (\ref{eq:06})). The{ }blend of binders OPC/lime 50/50\% {contributed} to the {increase of} MCV {to} 1.85{ }for soil \emph{B} (Equation~(\ref{eq:01})). For~soil type \emph{A}, there was an adjusted $R^2$ value of 0.999, i.e.,~a very low variability. {The} GGBFS {had the least effects} on soil compaction ({Figure} \ref{fig:05}).
...
\begin{figure}[H]
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \label{fig:05b}
     \end{subfigure}
        \caption{MCV for various binders blended with soil: (\textbf{a}) type \emph{A} {(Brom\"olla)}; (\textbf{b}) type \emph{B} {Petersborg}.}
        \label{fig:05}
\end{figure}
\unskip

\subsection{UCS}
The UCS {was tested for types} \emph{A} and \emph{B} on days 7, 28 and 90, while soil type \emph{C} was tested on days 28 and 90. 

\subsubsection{Measurements of UCS on 7th day}
Because the soil types varied in clay content, we assumed that {the performance of} lime {differed} in soil types \emph{A, B} and \emph{C}, which was proved by {Figures} \ref{fig:06}--\ref{fig:08}. The 7-day UCS {values evaluated} for{ }types \emph{A} and \emph{B} are given in Equations (\ref{eq:08}) and (\ref{eq:09}).
...
{The }UCS for soil \emph{A} and \emph{B} cured for 7 days {is shown in Figure} \ref{fig:06}. {The comparative analysis of soil types showed that } type \emph{A} {had} a higher strength compared to that of \emph{B} and \emph{C}. Furthermore, it also {demonstrated} a significantly larger {extent of variations in UCS values} compared to type~\emph{B}. 
...
Indeed, soil of type \emph{A} had the UCS varying within 2--6 MPa, while the UCS of type \emph{B} {ranged} from 0.8 {up to} 2.7 MPa. {The} 7-day UCS for type \emph{A} showed {a} significant full quadratic regression model, i.e.,~all effects from all binders were considerable. We also noted that OPC had the largest impact on the UCS of soil, while GGBFS had the lowest impact on UCS for both soil types ({Figure} \ref{fig:06}). After~a 7-day curing period, the~OPC-lime blend had a negative effect on UCS, i.e., this part of the blend reduced the UCS. For~soil of type \emph{B}, the~UCS on the 7th day of curing resulted in a linear model %Please check intended meaning has been retained -- Answer: yes, correct.
...
The UCS of stabilised soil of types \emph{A, B} and \emph{C} on~the 28th day of the curing period are presented in the subplots of {Figure} \ref{fig:07}. The~UCS of soil in kPa for soil types \emph{A, B} and \emph{C} on the 28th day of curing is summarised in Equations~(\ref{eq:10})--(\ref{eq:12}). Here \emph{C, L} and \emph{S} indicate OPC, lime and GGBFS, and~the combinations \emph{CS} and \emph{LS} are their mixes.
...
\vfill \vspace{1mm}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \label{fig:07c}
     \end{subfigure}
        \caption{UCS strength for soil--binder mixtures cured for 28 days; {soil types}: (\textbf{a}) \emph{A}; (\textbf{b}) \emph{B}; (\textbf{c}) \emph{C}.}
        \label{fig:07}
\end{figure}

The OPC--lime blend{ }was not significant and was removed from the regression for{ }type \emph{A}. On~day 28 of curing{, the} OPC remained the dominating element for the UCS of soil. However, GGBFS was larger compared to the lime component (Equation~(\ref{eq:08})). As for type \emph{B}, samples{ }received significant effects {on day 28} (Equation (\ref{eq:11})){, as in} the {results on day 7} (Equation (\ref{eq:09})). Nevertheless, the~{effects from the blend} ``slaked lime/steel slag'' {were more notable} compared to the other blends{, which} suggests that lime was well suited to soil type \emph{B} as a binder. The{ additional }points were {used} to {check} the results in the UCS test {on day 28} of curing{, }which {showed an acceptable} $R^2$ value. The~effects from lime were not significant for soil type \emph{C}. The~{effects} from{ lime as a binder were smaller} (Equation (\ref{eq:12})) {compared} to{ }types \emph{A} and \emph{B} and could be considered as minor {ones}. Such minor effect from the slaked lime was due to the low clay content in soil type \emph{C}. Other factors could be the short curing time, as the effects from lime were time-dependent, see \mbox{{Figure} \ref{fig:01}}. Certainly, slaked lime required a longer curing time to result in a reaction similar to that from cement and steel~slag.  

\subsubsection{Measurements of UCS on 90th~Day}
The {results of the} UCS {values evaluated on day 90 of curing are shown} in {Figure} \ref{fig:08}. Equation~(\ref{eq:13}) {described the} type \emph{A} with the least effective binder{ }as OPC--GGBFS. This can be compared with Equations~(\ref{eq:08}) and (\ref{eq:10}) showing significant effects from the OPC--GGBFS blend. The~{ lime--GGBFS mixture showed a} notable correlation {on day 90 of} curing{. }The {effects from} the lime--GGBFS{ }mixture with a 50/50\%{ were more significant} compared to {the} single binder of slaked lime. The~effects from the blends of OPC/lime and lime/GGBFS are shown for{ }type \emph{B} (Equation (\ref{eq:14})). The~maximum UCS {was} in {the} range between the UCS of the OPC/lime and lime/GGBFS {mixtures} ({Figure} \ref{fig:08}). {GGBFS had the highest impact on the UCS of soil type \emph{C} on day 90 of curing}, (Figure \ref{fig:08} and Equation~(\ref{eq:14})).
...
\vfill \vspace{1mm}
     ...
     \begin{subfigure}[b]{0.45\textwidth}
         ...
         \label{fig:08c}
     \end{subfigure}
        \caption{UCS for soil types (\textbf{a}) \emph{A}, (\textbf{b}) \emph{B}, (\textbf{c}) \emph{C} cured for 90 days with various~binders.}
        \label{fig:08}
\end{figure}
	
The clay content {was the lowest} in soil type \emph{C} (6\%), which suggested that slaked lime had a lesser effect on the UCS of {this type of soil} (Equation~(\ref{eq:15})). The~minimum 7-day UCS of the cube specimens %Please check intended meaning has been retained
...
\begin{enumerate}
   ...
   \item {A slow} increase in {the} UCS {values} for the lime-stabilised specimens was explained by the low clay content in soil {classified as} type \emph{A}. {The} slaked lime performed better as a binder{, }and had a stronger {effect on} soil type \emph{B}.
 ...
 \end{enumerate}
	...
	We noted positive effects from the blends GGBFS/lime on UCS gain in soils {of} \emph{A} and \emph{B} classes. Moreover, the~GGBFS/lime {blend had stronger effects on soil strength} compared to{ }GGBFS or lime. In~soil {of class} \emph{B}, the~{mixture} of GGBFS and slaked lime {performed better than }OPC, {because the} blend of GGBFS/lime did not affect the compatibility of soil as {an} OPC binder, inasmuch as all the specimens were produced with an identical compaction approach. Second, {the amount of} clay in {specimens of class} \emph{B} was enough {for effective soil stabilisation}, while in soil {of class} \emph{A}, it was too low {for} a reaction{ }when adding OPC{. The~vibratory compaction method demonstrated} similar {results} in {the} dry density values of soil{ }to the specimens compacted by {the }MCA {method}. Therefore, it {is} possible to achieve these values {in situ}, as~the maximum MCV was lower than~14.
...
{Second, we demonstrated that} well-adjusted techniques could significantly improve {the} time-consuming workflow{ in civil engineering works }and reduce the {total} economic costs of project implementation. For~example, in~{this} study, {we showed that }using pure OPC for the stabilisation of soil types \emph{A} and \emph{B} resulted in the best effects on {the }MCV{, because~}soil stabilised by OPC {had} a higher compaction energy due to a much higher{ }stiffness. 
...
%
\informedconsent{Not applicable.}%MDPI: To AE: Please confirm if  IRBS and ICS can be removed

\dataavailability{Not applicable.} 

\acknowledgments{We thank the two anonymous reviewers for their constructive comments and careful reading of this manuscript during rigorous peer review process. The~study was supported by the NUTEK---Swedish Agency for Economic \& Regional Growth, Peab AB construction and civil engineering enterprise and Merox. We thank Cementa (HeidelbergCement Group Northern Europe) for the provided cement binder and~Nordkalk for the limestone binder. The~authors also acknowledge help from colleagues from the Lund University, Chalmers Institute of Technology and the Swedish Geotechnical Institute with laboratory assistance and advices regarding the experimental tests.}
...
\newblock In Proceedings of the Grouting and Deep Mixing. In Proceedings of the
  Fourth International Conference on Grouting and Deep Mixing, New Orleans, LA, USA, 15--18 February 2012; pp.
  512--521.%MDPI: All highlights: We added the location and date of the conference. Please confirm. -- Yes, correct, we confirm. 
...
\newblock In Proceedings of the {Third International Conference on Grouting and
  Ground Treatment}, New Orleans, LA, USA, 10--12 February 2003; pp. 562--574.
...
\newblock In Proceedings of the 5th EEGS-ES Meeting, Budapest, Hungary, 6--9 September 1999; pp.
  1--2.
...
\newblock In Proceedings of the Fourth Geo-China International Conference, Shandong, China, 25--27 July 2016; pp. 69--76.
...
\newblock In Proceedings of the Fourth Geo-China International Conference, Shandong, China, 25--27 July 2016; pp. 53--60.
...
\newblock In Proceedings of the Geo-Shanghai 2014, Pavement
  Materials, Structures, and Performance, Shanghai, China,  26--28 May 2014; pp. 364--374.
...
\newblock In Proceedings of the {1st First International Symposium on Pavement and Geotechnical
  ...
  Engineering for Transportation'}, Nanchang, China, 5--7 June 2011; pp. 68--75.
...
\newblock In Proceedings of the Geo-Denver 2000, Soil-Cement and
  Other Construction Practices in Geotechnical Engineering, Denver, CO, USA, 5--8 August 2000; pp.
  26--35.
